...
//                }
     ...
     */
    public synchronized boolean sendRaw(String rawCommand){
     ...
     */
    public synchronized List<String> getStickers(String filterCommand) {
        ...
        printDebug("Track List has "+retList.size()+" elements!");
        return retList;
    }
     ...
     * @return True if server responded with ok
     */
    public synchronized boolean playTrackList(List<String> tracks) {
        if (null == tracks) {
            return false;
        }
        startCommandList();
    ...
    public synchronized int getRating(String trackUri){
        int retVal = -1;
        if(!isConnected()){
            return -1;
        }
        ...
        while (isConnected() && response != null && !response.startsWith("OK") && !response.startsWith("ACK")) {
            if (response.startsWith(MPDResponses.MPD_RESPONSE_STICKER)) {
                ...
                if(3 <= respParts.length){
                    retVal = Integer.parseInt(respParts[2]);
                }
            }
            response = readLine();
        }

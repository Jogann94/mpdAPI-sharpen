...
import org.gateshipone.malp.Logging_additionals.LoggingConnection;
import org.gateshipone.malp.R;
import org.gateshipone.malp.Logging_additionals.LoggingCommandHandler;
...
public class ConnectionManager extends MPDConnectionStateChangeHandler {
    ...
    public static void setParameters(MPDServerProfile profile, Context context) {
        ...
        //M.Schleinkofer 11.06.2017
        setRatingParams(profile, context);
    }

    //M.Schleinkofer 11.06.2017
    public static void setRatingParams(MPDServerProfile profile, Context context){
        if(null == profile){
            return;
        }
        LoggingConnection.getInstance().setServerParameters(profile.getRatingHostname(), profile.getRatingPort());
    }
    ...
    public static void reconnectLastServer(Context context) {
        ...
        //M.Schleinkofer 11.06.2017
        if(null != mConnectionManager.mServerProfile && mConnectionManager.mServerProfile.isRatingEnabled()) {
            LoggingCommandHandler.connectToServer();
        }
    }
    ...
    public static void disconnectFromServer() {
        ...
        //M.Schleinkofer 11.06.2017
        if(mConnectionManager.mServerProfile.isRatingEnabled()){
            LoggingCommandHandler.disconnectFromServer();
        }
    }

...
public class MPDProfileManager {
    ...
    public List<MPDServerProfile> getProfiles() {
        ...
        if ( cursor.moveToFirst() ) {
            do {
                ...
                //M.Schleinkofer 03.05.2017
                String ratingHostname = cursor.getString(cursor.getColumnIndex(MPDServerProfileTable.COLUMN_RATING_HOSTNAME));
                int ratingPort = cursor.getInt(cursor.getColumnIndex(MPDServerProfileTable.COLUMN_RATING_PORT));
                boolean ratingEnabled = cursor.getInt(cursor.getColumnIndex(MPDServerProfileTable.COLUMN_RATING_ENABLED)) == 1;
                profile.setRatingHostname(ratingHostname);
                profile.setRatingPort(ratingPort);
                profile.setRatingEnabled(ratingEnabled);
    ...
    public void addProfile(MPDServerProfile profile) {
        ...
        values.put(MPDServerProfileTable.COLUMN_RATING_HOSTNAME, profile.getRatingHostname());
        values.put(MPDServerProfileTable.COLUMN_RATING_PORT, profile.getRatingPort());
        values.put(MPDServerProfileTable.COLUMN_RATING_ENABLED, profile.isRatingEnabled());
